...
export const Sidebar: React.FC<SidebarProps> = ({ activeView, onViewChange, isOpen, setIsOpen, onSync, isSyncing, onClassifyAll }) => {
  return (
    <>
      <div
        className={`fixed inset-0 bg-slate-900/50 z-20 lg:hidden transition-opacity ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        onClick={() => setIsOpen(false)}
      />
      <aside className={`fixed left-0 top-0 h-full w-64 bg-white border-r border-slate-200 flex flex-col z-30 transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <nav className="flex-1 px-4 py-4 space-y-1">
          ...
          <NavItem
            ...
            onClick={() => { onViewChange('transactions'); setIsOpen(false); }}
          />
          <NavItem
